...
const ProductCard = ({ product }) => {
	...
	return (
		<StyledWrapper>
			<div className="card">
				<img className="card-img object-center object-cover" src={product.images.length === 0 ? '' : product.images[0].url} alt='no image' />
				<div className="card-info">
					<p className="text-title">{product.title}</p>
					<p className="text-body">{product.description}</p>
				</div>
				<div className="card-footer">
					<span className="text-title">{`${getCurrencySymbol()} ${product.price.amount}`}</span>
